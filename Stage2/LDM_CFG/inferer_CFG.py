...
class DiffusionInferer(Inferer):
    ...
    def __init__(self, scheduler: Scheduler, guidance) -> None:  # type: ignore[override]
        super().__init__()
    ...
    def sample(
        ...
        # Compute encoded condition and uncondition
        cond_enc = diffusion_model.resnet_10(conditioning)
        uncond_enc = diffusion_model.resnet_10(conditioning.fill_(118))
        ...
        for t in progress_bar:
            ...
            if mode == "concat" and conditioning is not None:
                ...
                cond_output = diffusion_model(
                    image, timesteps=torch.Tensor((t,)).to(input_noise.device), context=cond_enc
                )

                uncond_output = diffusion_model(
                    image, timesteps=torch.Tensor((t,)).to(input_noise.device), context=uncond_enc
                )

                model_output = uncond_output + self.guidance * (uncond_output - cond_output)
