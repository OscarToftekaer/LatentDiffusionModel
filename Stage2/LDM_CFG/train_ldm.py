...
sys.path.append('/homes/marcus/Dokumenter/LDM_fdg/CFG')
from ldm_pinaya_CFG import DiffusionModelLightning
...
class LatentLabelDataset(Dataset):
    def __init__(self, folder_path):
        ...
        self.seg_files = sorted([f for f in os.listdir(folder_path) if f.startswith("seg_") and f.endswith(".npy")])
    ...
    def __getitem__(self, idx):
        ...
        seg_path = os.path.join(self.folder_path, self.seg_files[idx])
        ...
        seg = np.load(seg_path).astype(np.int64)  # Position information

        # Convert `labels` to a list
        labels = labels.astype(int).tolist() # Ensure labels are a list, not a NumPy array
        
        return {"labels": labels, "latent": latent, "seg": seg}
...
def custom_collate_fn(batch):
    ...
    seg = torch.stack([torch.tensor(item["seg"], dtype=torch.float32) for item in batch])  # Stack positions
    return {"labels": labels, "latents": latents, "seg": seg}
...
def main():
    ...
    parser.add_argument('--config_path', type=str, default='config_folder/config_ldm_CFG.yaml')
    ...
    checkpoint_best = ModelCheckpoint(
        ...
        dirpath='/depict/users/marcus/shared/LDM_fdg_checkpoints/ldm_fdg_pinaya_CFG_it2/',    # Directory to save the checkpoints
        filename='fdg_LDM_pinaya_it2-best-{epoch:02d}-{val_loss:.8f}',
    ...
    checkpoint_periodic = ModelCheckpoint(
        dirpath='/depict/users/marcus/shared/LDM_fdg_checkpoints/ldm_fdg_pinaya_CFG_it2/',
        filename='fdg_LDM_pinaya_it2-periodic-{epoch:02d}',
        every_n_epochs=1,
        save_top_k=-1
    )
